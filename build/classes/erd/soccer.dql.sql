...
-- 키와 몸무게가 없으면 "0" 표시
-- 키 내림차순
SELECT PLAYER_NAME 선수명,CONCAT(NVL2(HEIGHT,HEIGHT,0),'cm') 키,CONCAT(NVL2(WEIGHT,WEIGHT,0),'kg') 몸무게
FROM PLAYER
WHERE TEAM_ID LIKE 'K02'
ORDER BY 2 DESC
;
...
-- BMI지수 
-- 키 내림차순
SELECT PLAYER_NAME 선수명, CONCAT(NVL2(HEIGHT,HEIGHT,0),'cm')키,
...
SELECT T.TEAM_NAME 팀명, STADIUM_NAME 스타디움이름,D.AWAYTEAM_ID 원정팀 , SCHE_DATE 스케줄
FROM TEAM T 
    JOIN STADIUM S
        USING (STADIUM_ID)
    JOIN SCHEDULE D
...
-- 스타디움, 경기날짜를 구하시오
-- 연고지와 팀이름은 간격을 띄우시오.
SELECT P.PLAYER_NAME 선수명, P.POSITION 포지션, T.REGION_NAME||' '||T.TEAM_NAME 팀명, S.STADIUM_NAME 스타디움, SCHE_DATE 스케줄날짜
FROM TEAM T 
    JOIN PLAYER P
        USING (TEAM_ID)
    JOIN STADIUM S
...
-- 홈팀 이름과 원정팀 이름을
-- 구하시오
SELECT S.STADIUM_NAME 경기장, D.SCHE_DATE 경기날짜, 
       CONCAT(CONCAT(T.REGION_NAME,' '),T.TEAM_NAME) 홈팀,
       (SELECT TEAM_NAME 
       FROM TEAM 
       WHERE TEAM_ID LIKE D.AWAYTEAM_ID) 어웨이팀, 
...
-- 팀ID, 팀명, 평균키 추출
SELECT P.TEAM_ID 팀아이디, T.TEAM_NAME 팀명, ROUND(AVG(HEIGHT))평균키
FROM PLAYER P 
    JOIN TEAM T
...
--DB가 클수록 절을 줄일수 있으면 퍼포먼스가 좋아진다.
SELECT P.PLAYER_NAME 선수명, P.POSITION 포지션, P.BACK_NO 백넘버
FROM PLAYER P 
    JOIN (SELECT TEAM_ID
                    FROM PLAYER
                    WHERE PLAYER_NAME LIKE '이현') T
        USING (TEAM_ID)
ORDER BY 1,2,3
;
...
SELECT TEAM_ID,
       NVL(SUM(CASE WHEN POSITION LIKE 'FW' THEN 1 END),0)PW,
       NVL(SUM(CASE WHEN POSITION LIKE 'MF' THEN 1 END),0)MF,
       NVL(SUM(CASE WHEN POSITION LIKE 'DF' THEN 1 END),0)DK,
...
-- 절대값으로 출력하기
SELECT D.SCHE_DATE, 
       CONCAT(CONCAT(T.TEAM_NAME,'-'),TT.TEAM_NAME)팀들,
       CONCAT(CONCAT(D.HOME_SCORE,'-'),D.AWAY_SCORE) 점수,
       ABS(D.HOME_SCORE-D.AWAY_SCORE)점수차
FROM (SELECT TEAM_ID, TEAM_NAME 
      FROM TEAM)T
    JOIN (SELECT SCHE_DATE,HOME_SCORE,
          AWAY_SCORE,HOMETEAM_ID,AWAYTEAM_ID
          FROM SCHEDULE 
          WHERE SCHE_DATE LIKE '201208%')D
        ON T.TEAM_ID LIKE D.HOMETEAM_ID
    JOIN (SELECT TEAM_ID,TEAM_NAME
          FROM TEAM)TT
        ON TT.TEAM_ID LIKE D.AWAYTEAM_ID
ORDER BY D.SCHE_DATE, ABS(D.HOME_SCORE-D.AWAY_SCORE) DESC
;

SELECT D.SCHE_DATE,
       CONCAT((SELECT T.TEAM_NAME 
              FROM TEAM T
              WHERE T.TEAM_ID LIKE D.HOMETEAM_ID),
             (SELECT TT.TEAM_NAME
              FROM TEAM TT
              WHERE TT.TEAM_ID LIKE D.AWAYTEAM_ID))팀들,
       CONCAT(CONCAT(D.HOME_SCORE,'-'),D.AWAY_SCORE) 점수,
       ABS(D.HOME_SCORE-D.AWAY_SCORE)점수차
FROM TEAM T
    JOIN (SELECT SCHE_DATE,HOME_SCORE,
...
FROM (SELECT STADIUM_ID, STADIUM_NAME
      FROM STADIUM) S 
    JOIN (SELECT STATIUM_ID,SCHE_DATE
          FROM SCHEDULE
          WHERE SCHE_DATE BETWEEN '20120501' AND '20120602') D
...
SELECT T.TEAM_NAME 팀명, P.PLAYER_NAME 선수명, P.HEIGHT 키, 
       ...
       FROM PLAYER PP
       WHERE P.TEAM_ID LIKE PP.TEAM_ID),2)평균키
FROM PLAYER P
    JOIN TEAM T
        ON P.TEAM_ID LIKE T.TEAM_ID
ORDER BY 4 DESC
...
SELECT T.TEAM_NAME 팀명, 
       ...
       P.PLAYER_NAME 선수명,
       P.HEIGHT 키
FROM (SELECT POSITION,BACK_NO,PLAYER_NAME,HEIGHT,TEAM_ID
      FROM PLAYER
      WHERE POSITION IN ('GK','MF'))P
        JOIN (SELECT TEAM_NAME, TEAM_ID
        FROM TEAM
